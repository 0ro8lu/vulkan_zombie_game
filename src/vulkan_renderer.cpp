...
VulkanRenderer::VulkanRenderer(GLFWwindow* window, uint32_t max_layers, uint32_t max_num_objects, uint32_t max_frames_flight)
    : window(window), max_layers(max_layers), max_num_objects(max_num_objects), max_frames_flight(max_frames_flight){
...
void VulkanRenderer::createDescriptorSetLayout() {
    VkDescriptorSetLayoutBinding saticUboLayoutBinding{};
    saticUboLayoutBinding.binding = 0;
    saticUboLayoutBinding.descriptorCount = 1;
    saticUboLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER;
    saticUboLayoutBinding.pImmutableSamplers = nullptr;
    saticUboLayoutBinding.stageFlags = VK_SHADER_STAGE_VERTEX_BIT;
    ...
    std::array<VkDescriptorSetLayoutBinding, 2> bindings = {saticUboLayoutBinding, samplerLayoutBinding};
    // std::array<VkDescriptorSetLayoutBinding, 3> bindings = {saticUboLayoutBinding, dynamicUboLayoutBinding, samplerLayoutBinding};
...
void VulkanRenderer::createGraphicsPipeline() {
    ...
    for(const auto& description : vertexAttributeDescription) {
        attributeDescriptions.push_back(description);
    }

    auto instanceAttributeDescription = InstanceData::getAttributeDescriptions();
    for(const auto& description: instanceAttributeDescription) {
        attributeDescriptions.push_back(description);
    }
...
void* VulkanRenderer::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, BufferType bufferType, VkBuffer& buffer, VmaAllocation& allocation) {
...
void VulkanRenderer::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VmaAllocationCreateFlagBits flags, VkImage& image, VmaAllocation& allocation, uint32_t layerCount) {
...
void VulkanRenderer::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout, VkImageSubresourceRange subresourceRange) {
...
void VulkanRenderer::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height, std::vector<VkBufferImageCopy> &bufferCopyRegions) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void VulkanRenderer::createTextureArray() {
    ...
    for(int i = 0; i < textureFiles.size(); i++) {
        ...
        stbi_uc* pixels = stbi_load(textureFiles[i].c_str(), &width, &height, &channels, STBI_rgb_alpha);
        ...
        // Calculate size for this texture
        imageSize = texWidth * texHeight * 4;  // 4 for RGBA

        // Append this texture's data to our vector
        textureData.insert(textureData.end(), pixels, pixels + imageSize);
    ...
    void* data = createBuffer(totalSize, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, BufferType::STAGING_BUFFER, stagingBuffer, stagingBufferAllocation);

    // bind buffer and copy data into it
    memcpy(data, textureData.data(), totalSize);

    // Setup buffer copy regions for array layers
	std::vector<VkBufferImageCopy> bufferCopyRegions;

	// To keep this simple, we will only load layers and no mip level
	for (uint32_t layer = 0; layer < textureFiles.size(); layer++)
	{
		...
		bufferCopyRegion.bufferOffset = imageSize * layer;
		bufferCopyRegions.push_back(bufferCopyRegion);
	}

    createImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT, VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT, textureImage, textureAllocation, textureFiles.size());

    VkImageSubresourceRange subresourceRange = {};
	subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
	subresourceRange.baseMipLevel = 0;
	subresourceRange.levelCount = 1;
	subresourceRange.layerCount = textureFiles.size();
    ...
    copyBufferToImage(stagingBuffer, textureImage, texWidth, texHeight, bufferCopyRegions);
	...
	view.subresourceRange.layerCount = textureFiles.size();
...
void VulkanRenderer::createIndexBuffer() {
    ...
    createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, BufferType::GPU_BUFFER, indexBuffer, indexBufferAllocation);
...
void VulkanRenderer::allocateInstanceBuffer(size_t bufferLen, VkBuffer& instanceBuffer, VmaAllocation& instanceBufferAllocation) {
    VkDeviceSize bufferSize = sizeof(InstanceData) * bufferLen;

    createBuffer(bufferSize, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, BufferType::GPU_BUFFER, instanceBuffer, instanceBufferAllocation);
}

void VulkanRenderer::uploadToInstanceBuffer(VkBuffer& instanceBuffer, const std::vector<InstanceData>& instanceData) {
...
void VulkanRenderer::deallocateInstanceBuffer(VkBuffer& instanceBuffer, VmaAllocation& instanceBufferAllocation){
...
void VulkanRenderer::createUniformBuffers() {
    ...
    dynamicAlignment = (sizeof(DynamicUBO) / alignMod ) * alignMod;
    if ( sizeof(DynamicUBO) % alignMod > 0 ) {
        dynamicAlignment += alignMod;
    }
    
    VkDeviceSize staticBufferSize = sizeof(StaticUBO);
    VkDeviceSize dynamicBufferSize = dynamicAlignment * max_num_objects;

    staticUniformBuffers.resize(max_frames_flight);
    // dynamicUniformBuffers.resize(max_frames_flight);
    
    staticUniformBuffersAllocation.resize(max_frames_flight);
    dynamicDynamicBuffersAllocation.resize(max_frames_flight);
    
    staticUniformBuffersMapped.resize(max_frames_flight);
    dynamicUniformBuffersMapped.resize(max_frames_flight);
    ...
    for (size_t i = 0; i < max_frames_flight; i++) {
        ...
        // dynamicUniformBuffersMapped[i] = createBuffer(dynamicBufferSize, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, BufferType::STAGING_BUFFER, dynamicUniformBuffers[i], dynamicDynamicBuffersAllocation[i]);
    }
}
...
void VulkanRenderer::createDescriptorPool() {
    ...
    poolSizes[0].descriptorCount = static_cast<uint32_t>(max_frames_flight);
    poolSizes[1].type = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER_DYNAMIC;
    poolSizes[1].descriptorCount = static_cast<uint32_t>(max_frames_flight);
    poolSizes[2].type = VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER;
    poolSizes[2].descriptorCount = static_cast<uint32_t>(max_frames_flight);

    VkDescriptorPoolCreateInfo poolInfo{};
    poolInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    poolInfo.poolSizeCount = static_cast<uint32_t>(poolSizes.size());
    poolInfo.pPoolSizes = poolSizes.data();
    poolInfo.maxSets = static_cast<uint32_t>(max_frames_flight);
...
void VulkanRenderer::createDescriptorSets() {
    std::vector<VkDescriptorSetLayout> layouts(max_frames_flight, descriptorSetLayout);
    VkDescriptorSetAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocInfo.descriptorPool = descriptorPool;
    allocInfo.descriptorSetCount = static_cast<uint32_t>(max_frames_flight);
    allocInfo.pSetLayouts = layouts.data();

    descriptorSets.resize(max_frames_flight);
...
void VulkanRenderer::recordCommandBuffer(VkCommandBuffer commandBuffer, std::vector<std::pair<VkBuffer, size_t>> renderData, uint32_t imageIndex) {
    ...
    vkCmdBeginRenderPass(commandBuffer, &renderPassInfo, VK_SUBPASS_CONTENTS_INLINE);
        ...
        for(int i = 0; i < renderData.size(); i++) {
            vkCmdBindVertexBuffers(commandBuffer, 1, 1, &renderData[i].first, offsets);
            vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSets[currentFrame], 0, nullptr);
            vkCmdDrawIndexed(commandBuffer, static_cast<uint32_t>(indices.size()), renderData[i].second, 0, 0, 0);
        }
...
void VulkanRenderer::draw(const std::vector<std::pair<VkBuffer, size_t>>& renderData) {
    ...
    recordCommandBuffer(commandBuffers[currentFrame], renderData, imageIndex);
    ...
    currentFrame = (currentFrame + 1) % max_frames_flight;
}
